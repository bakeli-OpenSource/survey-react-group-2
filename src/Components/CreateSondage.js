...
const CreateSondage = () => {
    ...
    {error && <Message variant="danger">{error}</Message>}
        ...
        <form onSubmit={addSondage} className="form-control my-4" id="survey" autoComplete="off">
            <h2 className="mt-0 mb-4 text-center text-2xl font-bold leading-9 tracking-tight text-gray-900 fw-bold">
                Créez Un Sondage
            </h2>

            <label className="font-semibold text-base text-black text-left fw-bold" htmlFor="surveyTitleField">
                Titre du sondage:
            </label>
            ...
            {option.map((question, questionIndex) => (
                <div key={questionIndex}>
                        ...
                        <div key={optionIndex} className="flex items-center mt-2">
                            ...
                            {question.options.length > 1 && (
                                <button
                                    ...
                                    className="ml-2 bg-danger text-white px-2 py-1 rounded"
                                >
                                    Supprimer
                                </button>    
                            )}
                    ...
                    <button
                        ...
                        className="mt-2 bg-danger text-white px-2 py-1 rounded"
                    >
                        Ajout option
                    </button>
                    <br/>
         ...
         <br/>
            <button
                type="submit"
                className="flex items-center justify-center h-11 px-6 w-64 bg-danger mt-6 mb-0 rounded font-semibold text-sm text-blue-100 hover:bg-blue-700 w-full"
            >
                Créer le sondage
            </button>
        </form>
